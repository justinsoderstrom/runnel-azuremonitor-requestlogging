...
The core flow: `BodyLoggerMiddleware` buffers the request/response streams, redacts sensitive data, and writes the captured bodies as **tags on the incoming request `Activity`** (the ASP.NET Core request span). The Azure Monitor OpenTelemetry exporter surfaces unrecognized activity tags as `customDimensions` on the `requests` record in Application Insights. There is no direct dependency on any telemetry client — if no listener is attached, `Activity.Current` is null and the middleware no-ops.
...
- `ISensitiveDataFilter` / `SensitiveDataFilter` — walks JSON bodies masking values by property name or regex; non-JSON bodies matching a regex are masked wholesale. Singleton, registered via explicit factory because it has two constructors.
